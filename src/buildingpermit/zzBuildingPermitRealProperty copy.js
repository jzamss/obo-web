...
import { Panel, Subtitle } from 'rsi-react-web-components';
...
const BuildingPermitRealProperty = (props) => {
  ...
  const findProperty = () => {
    const svc = Service.lookup( app.orgcode + ":OboOnlineService" );
    return svc.findLocation( {refno: self.refno}, (err, location) => {
      if (err) {
        setError(err);
      } else {
        setMode("view-lot");
        if(location.owner.address.type == 'local' ) {
          location.owner.resident = 1;
        }
        else {
          location.owner.resident = 0;
        }
        if( location.owner.address.city ) location.owner.address.citymunicipality = location.owner.address.city;
        if( location.owner.address.municipality ) location.owner.address.citymunicipality = location.owner.address.municipality;
        location.owner.ctc = {};      //allocate ctc
